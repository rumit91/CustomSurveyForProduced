...
function getData (modal) {
	var sessionID = $("body").attr("sessionID");
	$.getJSON("./php/getDataSection4.php", {sessionID: sessionID}, function (data) {
		if(data != null)
		{
			var i = 1;
			for(i=1; i<9; i++)
			{
				$("input.q"+i+"current[value='"+data["q"+i+"current"]+"']").attr('checked','checked');
				$("input.q"+i+"current[value='"+data["q"+i+"current"]+"']").parent().addClass("selected-bg");
			}
			var j = 1;
			for(j=1; j<9; j++)
			{
				$("input.q"+j+"desired[value='"+data["q"+j+"desired"]+"']").attr('checked','checked');
